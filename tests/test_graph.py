...
def test_merge_two_sources_fill_blanks_and_union_edges(store_snapshot):
    ...
    schema_graph = build_graph([store_snapshot, other])
    t = schema_graph.table("public.orders")
    ...
    assert schema_graph.relations("public.orders", "public.shipment")[0].kind == "catalog_relation"
    assert "revenue" in schema_graph.terms
    assert schema_graph.graph.has_edge("k:revenue", "c:public.orders.total_amount")
...
def test_stub_for_unknown_referenced_table():
    ...
    assert schema_graph.table("b") is not None and schema_graph.table("b").properties.get("stub") == "true"
...
def test_shortest_path_union_includes_bridge_tables(store_graph):
    ...
    assert len(paths) == 1
    assert [store_graph.graph.nodes[n]["fqn"] for n in paths[0]] == ["public.customer", "public.orders", "public.order_items", "public.products", "public.product_category"]
...
def test_weighted_paths_prefer_foreign_keys_over_lineage(store_snapshot):
    ...
    paths, _ = union_of_shortest_paths(schema_graph, ["public.customer"], ["public.products"])
    fq = [[schema_graph.graph.nodes[n]["fqn"] for n in p] for p in paths]
...
def test_ppr_matrix_matches_networkx(store_graph):
    ...
    ref = nx.pagerank(store_graph.graph, alpha=0.85, personalization=seeds, weight="weight", tol=1e-12, max_iter=2000)
...
def test_ppr_untouched_component_scores_exactly_zero():
    ...
    schema_graph = build_graph([snap])
    scores = personalized_pagerank(schema_graph, {tnode("alpha"): 1.0})
...
def test_relation_edges_carry_join_cost_and_ppr_affinity(store_snapshot):
    ...
    schema_graph = build_graph([store_snapshot])
    inf = schema_graph.graph.get_edge_data(tnode("public.orders"), tnode("public.audit_log"))
    fk = schema_graph.graph.get_edge_data(tnode("public.orders"), tnode("public.customer"))
    assert (inf["weight"], inf["affinity"]) == (2.5, 1.0)  # cost for path-finding, uniform flow for PPR
    assert (fk["weight"], fk["affinity"]) == (1.0, 1.0)
    by_cost = PPRMatrix(schema_graph, "weight").run({tnode("public.orders"): 1.0})
    by_affinity = PPRMatrix(schema_graph, "affinity").run({tnode("public.orders"): 1.0})
...
def test_user_snapshot_outranks_catalog_terms(store_snapshot):
    ...
    for order in ([store_snapshot, catalog, user], [user, catalog, store_snapshot]):
        schema_graph = build_graph(order)
        term = schema_graph.terms["revenue"]
...
def test_edges_and_terms_resolve_regardless_of_snapshot_order():
    ...
    for a_prio, b_prio in ((1, 2), (2, 1)):  # force both merge orders (same source_type ties on name)
        ...
        schema_graph = build_graph([a, b])
        assert schema_graph.table("customer").properties.get("stub") is None
        assert schema_graph.graph.has_edge("c:orders.customer_id", "c:customer.id")  # column-level FK resolved after both tables loaded
        glossary = {m for m in schema_graph.graph.neighbors("k:buyer") if schema_graph.graph["k:buyer"][m]["etype"] == "glossary"}
        assert glossary == {"c:customer.name"}  # no table-level fallback edge left from the stub
        assert schema_graph.relations("orders", "customer")[0].kind == "foreign_key"
...
def test_stub_flag_in_a_snapshot_is_not_trusted():
    ...
    schema_graph = build_graph([real, echoed])
    t = schema_graph.table("t")
...
def test_stub_replaced_when_real_table_arrives_incrementally():
    schema_graph = build_graph([SchemaSnapshot(source="a", source_type="ddl", tables=[Table(name="orders", columns=[Column(name="customer_id")])], edges=[Edge(kind="foreign_key", from_table="orders", to_table="customer", from_columns=["customer_id"], to_columns=["id"])])])
    assert schema_graph.table("customer").properties.get("stub") == "true"
    schema_graph.add_snapshot(SchemaSnapshot(source="b", source_type="duckdb", tables=[Table(name="customer", description="real", columns=[Column(name="id"), Column(name="name")])]))
    c = schema_graph.table("customer")
    assert c.properties.get("stub") is None and c.description == "real" and len(c.columns) == 2
    assert "a" in c.source and "b" in c.source
    assert schema_graph.graph.has_edge("t:orders", "t:customer") and schema_graph.graph.has_edge("c:orders.customer_id", "c:customer.id")
